...
export function Products() {
  ...
  return (
    <section id="productos" className="relative overflow-hidden bg-secondary/40 py-20 sm:py-28">
      <div className="relative mx-auto max-w-7xl px-6 sm:px-10">
        ...
        <div className="mt-10 grid gap-4 rounded-3xl border border-border bg-card p-6 shadow-sm sm:grid-cols-[1fr_auto] sm:items-center">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20"
          >
...
function ProductCard({ product, delay }: { product: Product; delay: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Placeholder: foto real del producto. Idealmente la misma toma
          (mismo fondo, misma luz) para las cuatro líneas, así las cards
          quedan consistentes entre sí. */}
      <PhotoPlaceholder
        label={`Foto de empaque — ${product.name}`}
        dimensions="800 × 600 px (4:3)"
        aspect="aspect-[4/3]"
        className="w-full rounded-none border-x-0 border-t-0 p-0 shadow-none"
      />
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="group/btn mt-5 inline-flex items-center justify-center gap-2 rounded-full border border-border px-5 py-2.5 text-[13px] font-semibold text-foreground transition-all hover:border-primary/50 hover:bg-primary hover:text-primary-foreground"
        >
          {product.cta}
